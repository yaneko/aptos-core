...
use crate::{
    ...
    protocols::{
        ...
        wire::messaging::v1::{
            metadata::{MessageMetadata, MultiplexMessageWithMetadata, NetworkMessageWithMetadata},
            ...
            loop {
                futures::select! {
                    message_with_metadata = stream.select_next_some() => {
                        ...
                        // Update the metadata network wire send time
                        message_metadata.update_network_send_time();
            ...
            Err(err) => match err {
                ReadError::DeserializeError(_, _, ref frame_prefix) => {
                    ...
                    let message_with_metadata = NetworkMessageWithMetadata::new(
                        MessageMetadata::new(ProtocolId::Unknown, None),
                        message,
                    );
