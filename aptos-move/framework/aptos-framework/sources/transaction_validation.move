module aptos_framework::transaction_validation {
    ...
    fun check_for_replay_protection_orderless_txn(
        ...
        let nonce_already_exists = nonce_validation::check_and_insert_nonce(sender, nonce, txn_expiration_time);
        assert(!nonce_already_exists, error::invalid_argument(PROLOGUE_NONCE_ALREADY_USED));
    ...
    fun prologue_common(
        ...
        assert!(txn_sequence_number.is_some() || nonce.is_some(), error::invalid_argument(PROLOGUE_NO_REPLAY_PROTECTOR_PROVIDED));
        assert!(txn_sequence_number.is_none() || nonce.is_none(), error::invalid_argument(PROLOGUE_BOTH_SEQ_NUMBE_AND_NONCE_PROVIDED));
        ...
        if (txn_sequence_number.is_some()) {
