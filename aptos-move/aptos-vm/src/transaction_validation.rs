    ...
    let (prologue_function_name, args) = if let (Some(fee_payer), Some(fee_payer_auth_key)) = (
        ...
        if features.is_orderless_txns_enabled() {
            ...
            let txn_sequence_number = match txn_replay_protector {
                ...
                ReplayProtector::Nonce(_) => Err(VMStatus::error(StatusCode::ORDERLESS_TXNS_NOT_SUPPORTED_YET, Some("Orderless transactions not yet supported".to_string()))),
            }?;
            let args = vec![
                MoveValue::Signer(txn_data.sender),
...
pub(crate) fn run_multisig_prologue(
    ...
    let provided_payload = match executable {
        TransactionExecutable::EntryFunction(entry_function) => {
            bcs::to_bytes(&MultisigTransactionPayload::EntryFunction(entry_function)).map_err(|_| unreachable_error.clone())?
        }
        ...
        TransactionExecutable::Script(_) => {
            ...
            return Err(VMStatus::error(
                StatusCode::MULTISIG_TRANSACTION_SCRIPT_PAYLOAD_NOT_SUPPORTED,
